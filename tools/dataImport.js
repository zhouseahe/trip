...
var base = ['A','B','C','D','E','F','G','H']; // 名称  ， 区域[] （ 景区 ），地址 ， 房型[]， 电话 ，联系人 ， 设施 ， 简介 ，省 ， 搜索
    ...
    execute : function(name,cb){
        ...
        for(var i = 1 ; i <= range ; i++){
            var topic = {title:worksheet[base[0]+i].v,erea:worksheet[base[1]+i].v,address:worksheet[base[2]+i].v,
                ...
                facility:worksheet[base[6]+i].v,content:worksheet[base[7]+i].v,search:worksheet[base[0]+i].v + worksheet[base[1]+i].v}
            // insert topic or  insert topics
            data.push(topic);
            topicService.insertMany(data,function(err,data){
                if(err){
                    console.log(err);
                    return ;
                }
